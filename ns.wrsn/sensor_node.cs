        ...
        public override void OnExPowerDown()
        {
           ...
           // printfx("Can't find MC {0}", this.tag);
            try
            {
                ...
                net_packet packet = new net_packet(this.tag, mc.tag, packetType.ChargeRequest, mc.tag);
                this.PowerConfig.charge_request_packets_tx += 1;
                mobile_charger fmc = mc as mobile_charger;
                fmc.OnPacketReceived(packet, this);

               /* if(mc.tag != string.Empty)
                {
                    /// to find the path between the MCV and the Node.
       ...
       private mobile_charger GetNearestMc()
        {
            ...
            if (this.MobileChargers != null && this.MobileChargers.Count > 0) 
            {
                ...
                {
                    var node = mc.Value;
                    double dist = this.edistance(mc.Value);
                    if (node.PowerConfig.BatteryCapacity > 300)
                    {
                        ...
                        {
                            //
                            min_qlen = node.GetQueueLen() * dist;
                            i++;
                            mc_id = node.tag;
                            continue;
                        }
                        //node.GetQueueLen() * 
                        double qlen = node.GetQueueLen() * dist;
